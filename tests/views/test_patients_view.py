...
def test_add_patient(client, app):
    ...
    assert response.json == {"id": 1, "name": "John Doe", "problem": "High fever", "ward_id": 2, "doctor_id": 3}
    patient_service_mock.add_patient.assert_called_once_with("John Doe", "High fever")
...
def test_update_patient(client, app):
    ...
    assert response.json == {"id": 1, "name": "Jane Doe", "problem": "Cold", "ward_id": 2, "doctor_id": 3}
...
def test_get_all_patients(client, app):
    ...
    assert response.json == [
        {"id": 1, "name": "John Doe", "problem": "High fever", "ward_id": 2, "doctor_id": 3},
        {"id": 2, "name": "Jane Doe", "problem": "Cold", "ward_id": 2, "doctor_id": 3},
    ]
...
def test_get_patients_by_ward(client, app):
    ...
    with app.container.patient_service.override(patient_service_mock):
        response = client.get("/api/patients/ward/2")

    assert response.status_code == 200
    assert response.json == [
        {"id": 1, "name": "John Doe", "problem": "High fever", "ward_id": 2, "doctor_id": 3},
        {"id": 3, "name": "Alice Brown", "problem": "Pneumonia", "ward_id": 2, "doctor_id": 3},
    ]
...
def test_get_patients_by_doctor(client, app):
    ...
    with app.container.patient_service.override(patient_service_mock):
        response = client.get("/api/patients/doctor/3")

    assert response.status_code == 200
    assert response.json == [
        {"id": 2, "name": "Jane Doe", "problem": "Cold", "ward_id": 2, "doctor_id": 3},
        {"id": 4, "name": "Bob Green", "problem": "Fracture", "ward_id": 2, "doctor_id": 3},
    ]
